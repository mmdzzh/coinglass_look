...
### 方案 E：直接 Python 部署（无 Docker）

适合不想用 Docker、希望直接以 Python + systemd + Nginx 运行的用户。

详细步骤见 [`deploy/DEPLOY_PYTHON.md`](deploy/DEPLOY_PYTHON.md)。

主要步骤：

```bash
# 安装 Python + PostgreSQL + Nginx
apt-get install -y python3.12 python3.12-venv python3-pip postgresql postgresql-contrib nginx git

# 创建数据库
sudo -u postgres psql -c "CREATE USER coinglass WITH PASSWORD 'YOUR_DB_PASSWORD';"
sudo -u postgres psql -c "CREATE DATABASE coinglass OWNER coinglass;"
